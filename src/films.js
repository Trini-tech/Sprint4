...
function bestFilmOfYear(array, year) {
  ...
  const amounts = moviesYear.map((a) => a.score)
  const maxScore = Math.max(...amounts);
  let result = moviesYear.filter((item) => item.score === maxScore);
  return result;
}

//EXERCICI COMPLERT: PULL REQUEST.
